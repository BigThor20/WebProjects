...
var setGameData = function(){
  ...
  p.innerText = 'Select level: ';
   ...
   level.addEventListener('change',function(){
    ...
    if(difficultyValue == 65){
      hintsNumber = 6;
    }else if(difficultyValue == 70){
       hintsNumber = 5;
...
var setHint = function(){
  ...
  hint.addEventListener('click', function(){
    var adevar=1;
    hintsCount++;
    while(adevar==1 && hintsCount < hintsNumber)
    {
...
function addClassToNthTd(){
  ...
  for(var i = 0; i < rows.length; i++){
    ...
    for(var j = 0; j < tdChildNode.length; j++){
      if(i<3){
        if(j<3)
          tdChildNode[j].className = 'first-square';
        else if (j<6) tdChildNode[j].className = 'second-square';
        else if (j<9) tdChildNode[j].className = 'third-square';
      }else if(i<6){
      	if(j<3)
      		tdChildNode[j].className = 'fourth-square';
      	else if(j<6) tdChildNode[j].className = 'fifth-square';
      	else if(j<9) tdChildNode[j].className = 'sixth-square';
      }else if(i<9){
      	if(j<3) tdChildNode[j].className = 'seventh-square';
      	else if(j<6) tdChildNode[j].className = 'eighth-square';
      	else if(j<9) tdChildNode[j].className = 'ninth-square';
      }
    }
  }
};
...
var validateSquare = function(element){
	var arrOfClasses = element.parentElement.className.split(/\s+/);
	var selectedClass = '';
	for(var i=0; i< arrOfClasses.length;i++){
		if(arrOfClasses[i].indexOf('-square') !== -1){
			selectedClass = '.' + arrOfClasses[i];
		}
	}
  var tds = document.querySelectorAll(selectedClass);
  var validationArr = [];
...
for(var i = 0; i < tds.length ; i++){
    ...
    tds[i].className += ' red-border-second'; 
   }
   else {
    $(tds[i]).removeClass('red-border-second');
   }
}
};
